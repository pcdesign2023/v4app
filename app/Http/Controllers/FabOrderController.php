            ...
            foreach ($request->all() as $key => $value) {
                if (str_starts_with($key, 'OFID_')) {
                    ...
                    $validator = Validator::make([
                        ...
                        'prod_name' => $request->input("prod_name_$index"),
                        'prod_ref' => $request->input("prod_ref_$index"),
                        ...
                        'prod_name' => 'required|string|max:250',
                        'prod_ref' => 'required|string|max:250',
                    ]);

                    if ($validator->fails()) {
                        $errors["Formulaire #$index"] = $validator->errors()->all();

                            \Log::error("Validation failed for form #$index", $validator->errors()->toArray()); // 👈 Add this
                            $errors["Formulaire #$index"] = $validator->errors()->all();
