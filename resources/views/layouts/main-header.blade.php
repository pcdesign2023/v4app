<!-- main-header opened -->
				...
				<div class="container-fluid">
					...
					<div  class="main-header-right">
						...
						<div class="nav nav-item  navbar-nav-right ml-auto">
							...
							<div style="display:none" class="dropdown nav-item main-header-message ">
							...
							<div  style="display:none" class="dropdown nav-item main-header-notification">
							...
							<div class="nav-item full-screen fullscreen-button">
							    <button id="enable-fullscreen" style="display:none !important;">Enable Fullscreen</button>
								<a class="new nav-link full-screen-link" href="#"><svg xmlns="http://www.w3.org/2000/svg" class="header-icon-svgs" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="feather feather-maximize"><path d="M8 3H5a2 2 0 0 0-2 2v3m18 0V5a2 2 0 0 0-2-2h-3m0 18h3a2 2 0 0 0 2-2v-3M3 16v3a2 2 0 0 0 2 2h3"></path></svg></a>
							</div>
							<div class="nav-item ">
							    <button id="enable-fullscreen" style="display:none !important;">Enable Fullscreen</button>
								<a class="new nav-link full-screen-link"  href="{{ route('logout') }}"
                                       onclick="event.preventDefault();document.getElementById('logout-form').submit();">                        <i class="fa fa-power-off" aria-hidden="true"></i></a>
                                       <form id="logout-form" action="{{ route('logout') }}" method="POST" style="display: none;">
                                        @csrf
                                    </form>	
							</div>
							<div style="display:none" class="dropdown main-profile-menu nav nav-item nav-link">
									...
									<div class="main-header-profile bg-primary p-3">
										<div class="d-flex wd-100p">
											...
											<div class="mr-3 my-auto">
												<h6>{{Auth::user()->name}}</h6><span>{{ Auth::user()->role ? Auth::user()->role->label : 'No Role Assigned' }}</span>
											</div>
										</div>
									</div>
							...
							<div style="display:none" class="dropdown main-header-message right-toggle">
    ...
    document.addEventListener("DOMContentLoaded", function () {
        ...
        if (closeToggle) {
            closeToggle.click(); // Simulate a click on the close button
        }
    });
</script>
<!-- <script>
    document.addEventListener("DOMContentLoaded", function () {
        let fullscreenBtn = document.querySelector(".full-screen-link");
        let enableFullscreenBtn = document.getElementById("enable-fullscreen");

        if (fullscreenBtn) {
            // Show the button temporarily for interaction
            enableFullscreenBtn.style.display = "none";

            enableFullscreenBtn.addEventListener("click", function () {
                fullscreenBtn.click(); // Now the user has interacted, click is allowed
                enableFullscreenBtn.style.display = "none"; // Hide button after activation
                console.log("Full-screen mode activated");
            });

            // Optionally auto-trigger the button if a user interacts anywhere on the page
            document.addEventListener("click", function triggerFullscreenOnce() {
                enableFullscreenBtn.click();
                document.removeEventListener("click", triggerFullscreenOnce); // Remove listener after first click
            });
        }
    });
</script> -->
<script>
    document.addEventListener("DOMContentLoaded", function () {
        ...
        // Detect if the device is a tablet or mobile (typically width <= 1024px)
        function isMobileOrTablet() {
            return window.innerWidth <= 1024; // Adjust breakpoint as needed
        }

        if (fullscreenBtn && isMobileOrTablet()) {
            enableFullscreenBtn.style.display = "none"; // Hide extra button

            enableFullscreenBtn.addEventListener("click", function () {
                fullscreenBtn.click();
                enableFullscreenBtn.style.display = "none";
                console.log("Full-screen mode activated on mobile/tablet.");
            });

            // Auto-trigger fullscreen mode on first user interaction
            document.addEventListener("click", function triggerFullscreenOnce() {
                enableFullscreenBtn.click();
                document.removeEventListener("click", triggerFullscreenOnce); // Remove listener after first click
            });
