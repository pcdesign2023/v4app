                ...
                <div class="modal-body">
                    ...
                    <div class="form-row">
                        ...
                        <div class="form-group col-md-6">
                            ...
                            <input type="hidden" name="client" id="client_hidden">
                    ...
                    <div class="form-row">
                        <div class="form-group col-md-6">
                            ...
                            <input type="hidden" name="prod_ref" id="prod_ref_hidden">
                        ...
                        <div class="form-group col-md-6">
                            ...
                            <input type="hidden" name="commande" id="commande_hidden">
        ...
        $(document).on('click', '.edit-mp-btn', function () {
            ...
            $.ajax({
                ...
                success: function (data) {
                    ...
                    $('[name="instruction"]').val(data.instruction);
                    $('[name="comment"]').val(data.comment);
                    $('#prod_ref_hidden').val(data.prod_ref);
                    $('#client_hidden').val(data.client);
                    $('#commande_hidden').val(data.commande);

                    // Lock all fields visually but keep them enabled (for submission)
                    $('#ofForm input:not([name="_token"], [name="priority"]), #ofForm textarea')
                        .prop('readonly', true)
                        .prop('disabled', false); // ensure they submit

// Disable select2 fields visually
                    $('#ofForm select').each(function () {
                        const name = $(this).attr('name');
                        if (name !== 'statut') {
                            $(this).prop('disabled', true); // disables client, commande, produit
                        }
                    });

// Enable statut and priority
                    $('[name="statut"]').prop('disabled', false);
                    $('[name="priority"]').prop('readonly', false).prop('disabled', false);

                    $('#ofModalLabel').text('Modifier la priorité / statut');
