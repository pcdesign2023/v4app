                            ...
                            <select name="Prod_ID" id="Prod_ID" class="form-control select2" required>
                                ...
                                @foreach ($products as $product)
                                    <option value="{{ $product->id }}"
                                            ...
                                            data-ref="{{ $product->ref_id }}"
                                            {{ $product->id == old('Prod_ID', $fabOrder->Prod_ID) ? 'selected' : '' }}>
                                        {{ $product->ref_id }} - {{ $product->product_name }}
                                    </option>
                                @endforeach
                            </select>
                            <input type="hidden" name="prod_name" id="prod_name" value="{{ old('prod_name', $fabOrder->prod_name) }}">
                            <input type="hidden" name="prod_ref" id="prod_ref" value="{{ old('prod_ref', $fabOrder->prod_ref) }}">
...
@section('js')
    ...
    <script>
        $(document).ready(function () {
            ...
            // 👇 New code for product fields
            $('#Prod_ID').change(function () {
                const selected = $(this).find(':selected');
                $('#prod_name').val(selected.data('name'));
                $('#prod_ref').val(selected.data('ref'));
            });
        });
    </script>
@endsection
